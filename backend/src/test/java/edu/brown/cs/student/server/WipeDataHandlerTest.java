...
public class WipeDataHandlerTest {
  ...
  /**
   * tests wipeData Handler without providing wipeData returns bad request response, includes
   * integration tests dealing with wiping data.
   */
  @Test
  public void testSuccess() throws IOException {
    ...
    assertNull(serverInfo.getUserData());
    query =
        "?usercsv={\"usercsv\":[[\"Title\"],[\"Crash%20Landing%20on%20You:%20Episode16\",\"3/26/23\"]]}";
    clientConnection = tryRequest("saveData" + query);
    assertEquals(200, clientConnection.getResponseCode());
    assertNull(serverInfo.getUserData());
    clientConnection = tryRequest("wipeData");
    ...
    assertNull(serverInfo.getUserData());
    query =
        "?usercsv={\"usercsv\":[[\"Title\",\"Date\"],[\"Crash%20Landing%20on%20You:%20Episode16\",\"3/26/23\"]]}";
    clientConnection = tryRequest("saveData" + query);
    assertEquals(200, clientConnection.getResponseCode());
    assertNotNull(serverInfo.getUserData());
