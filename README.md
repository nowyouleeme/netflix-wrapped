...
- `main`
  ...
  - `csv.data`
    - `Data` class to represent the important data structures. 1 being the user's csv as a 2d array
  ...
  - `reports` is the file that holds using the data from `components` in order to combine and make a json.
    ...
    - `MockRGenerator` generator that mocks generating a wrapped report
    - `MockWrappedJSON.json` a json that holds the mock wrapped report
    - `ReportGenerator` interface for pattern strategy. Any class that has a function for generating a wrapped report given a user csv implements this interface.
  - `server` directory holds the server file and the handlers.
    - `handlers` directory for the endpoint handlers
      - `SaveDataHandler` is the handler to deal with handling requests to the saveData endpoint. It handles saving a user's netflix watching history csv.
      - `WipeDataHandler` is the handler to deal with handling requests to the wipeData endpoint. It handles deleting a user's netflix watching history csv from our server
      - `WrappedHandler` is the handler to deal with handling requests to the wrapped endpoint. It handles generating a wrapped report containing the information the frontend will present (given the user csv in the server), and it returns a response containing the wrapped report.
    ...
    - `Server` class for running/starting the server with endpoints for saving user csv, generating wrapped, and wiping user data.
    - `ServerInfo` class representing the information we store in our server. This includes jsut the current user's csv.
  - `Test` testing.
...
- `test`
  - `server` package holding tests regarding calling the java backend server endpoints
    - `IntegrationTest`: holds integration tests for user flows calling multiple endpoints (saveData, wrapped, wipeData)
    - `MockUserCSVQuery`: class to represent mock user csv queries to the saveData enpoint
    - `SaveDataHandlerTest`: holds tests for calling on the saveDataHandler
    - `WipeDataHandlerTest`: holds tests for calling on the saveDataHandler
    - `WrappedHandlerTest`: holds tests for calling on the saveDataHandler
    - `WrappedResp`: record used in testing to parse response from wrapped endpoint
  ...
  - `components` is a subpackage with tests of the components in the `components` package, such as rendering and interactions with populated and unpopulated mock data that would be sent by the backend
    - `report` is a subpackage with tests of the components used in the Report page
      ...
      - `WrappedOverview.test` is a test ensuring that the `WrappedOverview` component is rendering properly
        ...
        1. tests that the WrappedOverview component with mock empty data from empty.json renders properly with the expected message that we weren't able to generate their overview
...
4. Install Anaconda for Python following the steps in the website. Then, run 'conda create -n [environment name]' to create a virtual environment for machine learning.
5. Activate the virtual environment by the command conda activate [environment name]. Install the following packages: tensorflow (conda install tensorflow), numpy (pip install numpy), pandas (pip install pandas), flask (pip install flask), and flask-restful (pip install flask-restful).
6. Enter the `backend-ml` directory using the command `cd backend-ml`. Then run command 'flask --app server run' to run the NVTI classifier embedded in the python flask server.
